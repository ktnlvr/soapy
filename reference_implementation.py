...
start_time = 0.0  # global variable
...
def xi_lmk(l, m, k):
    if (k - l) % 2 != 0:
        ...
        den = math.gamma(k - m + 1) * math.gamma(l - k + 1) * math.gamma((l + k - 1)/2 - l + 1)
        value = num / den
...
def compute_c_nlm(n, l, m, alpha_bl, beta_nbl, x_p, y_p, z_p, sigma):
    ...
    c_nlm *= prefactor * (2*sigma*sigma*math.pi)**(3/2)
    return c_nlm
...
if __name__ == "__main__":
    ...
    print(f"Elapsed time (μs): {elapsed_time:.6f}\n")
